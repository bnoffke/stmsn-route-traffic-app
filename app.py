...
st.set_page_config(page_title="Travel Time Comparison", layout="wide")
...
with st.sidebar:
    ...
    # Snap to first Monday on or after data_start, then split available weeks in half
...
if deltas.empty:
    ...
    for window in sorted(deltas["time_window"].unique()):
        ...
        for i, (_, row) in enumerate(w_df.iterrows()):
            ...
            direction_arrow = "▲" if delta_s > 0 else "▼"
            color = "red" if delta_s > 0 else "green"
            with cols[i]:
                st.metric(
